...
export default async function handler(req,res){
  ...
  try{
    ...
    return res.status(200).json({generatedAt:new Date().toISOString(),students,eventCount:events.length});
  }catch(err){
    console.error(err);return res.status(500).json({error:'Could not read tracking data'});
  }
}
