...
export default async function handler(req,res){
  ...
  try{
    ...
    const type=['registration','started','attempt'].includes(b.type)
      ? b.type
      : null;
    ...
    const group=clean(b.group,60);
    const studentKey=clean(b.studentKey,120);

    if(!name || !group || !studentKey){
      return res.status(400).json({error:'Missing learner identity'});
    }

    const mission=b.mission && /^m[1-7]$/i.test(String(b.mission))
      ? String(b.mission)
      : null;
    ...
    const path=
      `${PREFIX}${safeSegment(group)}/${safeSegment(studentKey)}/${Date.now()}-${id}.json`;
